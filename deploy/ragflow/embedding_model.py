...
# SiliconFlow can occasionally take longer than the 30-second timeout that
# was hard-coded in RAGFlow v0.26.4.  Keep the deployment default at 100
# seconds while allowing operators to tune it through the container
# environment without editing this vendored module again.
DEFAULT_SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS = 100.0


def _siliconflow_embedding_timeout() -> float:
    """Return a validated request timeout for SiliconFlow embeddings.

    The value is read for each request so a running worker can pick up an
    environment override after a restart.  Invalid or non-positive values
    fall back to the safe 100-second default instead of failing an embedding
    request before it reaches the provider.
    """
    raw_timeout = os.getenv("SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS")
    if raw_timeout is None or not raw_timeout.strip():
        return DEFAULT_SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS
    try:
        timeout = float(raw_timeout)
    except (TypeError, ValueError):
        logger.warning(
            "Invalid SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS=%r; using %.1fs",
            raw_timeout,
            DEFAULT_SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS,
        )
        return DEFAULT_SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS
    if timeout <= 0:
        logger.warning(
            "SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS must be positive; using %.1fs",
            DEFAULT_SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS,
        )
        return DEFAULT_SILICONFLOW_EMBEDDING_TIMEOUT_SECONDS
    return timeout
...
class SILICONFLOWEmbed(Base):
    ...
    def _call(self, batch):
        ...
        response = requests.post(
            ...
            timeout=_siliconflow_embedding_timeout(),
        )
        return self._openai_http_embeddings(response)
