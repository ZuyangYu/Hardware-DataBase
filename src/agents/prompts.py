...
- 如果账本中某个未覆盖子问题存在 missing_evidence_types，tool_name 必须匹配缺失证据类型：document_text 用 document_rag；spreadsheet_table 用 spreadsheet_semantic 或 spreadsheet_cell；circuit_design 用 circuit_query；external_conversation 用 conversation_search。
...
- tool_name 从 document_rag / spreadsheet_semantic / spreadsheet_cell / circuit_query / conversation_search 中选；source_name 可选（跨语料时指定新源，不指定则广搜）。

返回 JSON：
{"status": "sufficient|partial_but_answerable|insufficient_need_more",
 "reason": "简短中文理由",
 "missing": ["具体缺失信息，如'server S-123 的规格'"],
 "suggested_queries": [{"query": "基于已发现实体的新检索句", "tool_name": "document_rag|spreadsheet_semantic|spreadsheet_cell|circuit_query|conversation_search", "source_name": "可选", "reason": "为什么这条查询能补上缺口"}]}
"""
...
- tool_name 必须与源类型匹配：spreadsheet 类源只能用 spreadsheet_semantic/spreadsheet_cell；文档类源用 document_rag；电路类源用 circuit_query；外部对话类(external_conversation)源只能用 conversation_search。
