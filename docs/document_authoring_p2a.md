...
- 上传模板只支持 `.xlsx`、`.xlsm` 和 `.docx`。先由规则型 OOXML 结构分析器在不把模板字节交给模型的前提下列出可写单元；可选的 LLM 只可对这份结构清单提出语义绑定建议。建议必须命中既有且可写的单元，无法解析或建议越界时进入人工处理。
- `TemplateVersion`、Workbook/DOCX Region 和 Template Unit Binding 必须先登记、再批准。确认时重新计算持久化模板字节的 hash，并要求它同时匹配分析记录和 TemplateVersion；随后在同一持久化激活事务中再次校验，防止确认与激活之间替换模板。生成时不会重新猜测可写区域。
...
- DOCX renderer 同样直接复制 OOXML 包，只可编辑 Renderer Policy 允许的 `word/document.xml`，并且只写入已登记、`validated_draft` 或 `deterministic_only` 的段落、表格单元或内容控件区域；锁定、公式、人工输入/批准及遗留示例区域永不可机写。其余 OOXML part、关系和主动内容均须保持不变并通过 Manifest 校验。
...
Streamlit 的“文档生成”页只允许选择当前用户具有项目成员权限的 Project、批准基线、模板和 Schema；它不会使用聊天知识库或会话状态作为生成输入。页面显示的工作单、Harness 节点、步骤、检索轮次和候选状态都从持久化 WorkOrder/HarnessRun/Checkpoint/Artifact 记录读取，不以临时会话状态作为运行事实。

## 当前阶段限制

- P2a 只实现 XLSM/XLSX 的确定性检查项；P2b 已在独立 Harness 中加入 Managed Writer、断言—证据校验、模板污染和跨单元一致性检查，以及受控 DOCX 输出，见 [document_authoring_p2b.md](document_authoring_p2b.md)。Markdown、外部 Agent Adapter 和 MCP 仍属于 P3。
